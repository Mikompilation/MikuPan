...
{
    VSOutput output;
    float4 clip = mul(projection, mul(view, input.aPos));
    output.position = MikuPanFixClipZ(clip);
    output.vUV = input.aUV;

    float3 normalVS = normalize(mul((float3x3)view, input.aNormal.xyz));
    float4 normal = float4(normalVS, 1.0);
    output.vNormal = normal;

    float4 viewPosition = mul(view, input.aPos);
    output.oViewPosition = viewPosition;
    output.oWorldPosition = input.aPos;
