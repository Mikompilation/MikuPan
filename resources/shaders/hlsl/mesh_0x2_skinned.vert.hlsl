...
// GPU 2-bone linear-blend skin for mesh type 0x2. See mesh_0xA_skinned.vert for
// the skinning derivation; the only difference is the tail, which mirrors
// mesh_0x2.vert (applies model / modelView / normalMatrix) so the result
// matches the old CPU-skinned 0x2 path exactly.
struct VSInput
{
...
VSOutput main(VSInput input)
{
    ...
    VSOutput output;
    float4 clip = mul(projection, mul(view, mul(model, aPos)));
    output.position = MikuPanFixClipZ(clip);
    output.vUV = input.aUV;

    float3 normalVS = normalize(mul(normalMatrix, aNormal.xyz));
    float4 normal = float4(normalVS, 1.0);
    output.vNormal = normal;

    float4 viewPosition = mul(modelView, aPos);
    output.oViewPosition = viewPosition;
    output.oWorldPosition = mul(model, aPos);
