...
bool MikupanAudioDecoder::readNextPrivateStream1PES()
{
    ...
    if (payloadLen >= 4 && payload[0] == 0xFF && payload[1] == 0xA0
...
void MikupanAudioDecoder::parseAccumulatedPayload()
{
    while (true)
    {
        if (!haveHeader_)
        {
            ...
            header_.type = rd_le32(&payloadAcc_[o]);
            o += 4;
            header_.rate = rd_le32(&payloadAcc_[o]);
            o += 4;
            header_.channels = rd_le32(&payloadAcc_[o]);
            o += 4;
            header_.interleave = rd_le32(&payloadAcc_[o]);

            haveHeader_ = true;

            payloadAcc_.erase(payloadAcc_.begin(),
                              payloadAcc_.begin() + o + 12);

            info_log("[AUDIO] type=%u rate=%u ch=%u interleave=%u",
                     header_.type, header_.rate, header_.channels,
                     header_.interleave);
