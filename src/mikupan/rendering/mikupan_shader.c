...
const char* shader_file_name[MAX_SHADER_PROGRAMS][3] = {
    ...
    {"./resources/shaders/sprite.vert",                      NULL,                                        "./resources/shaders/shadow_caster.frag"},
    ...
    {"./resources/shaders/shadow_silhouette.vert",           NULL,                                        "./resources/shaders/shadow_silhouette.frag"},
};
...
{
    if (idx < 0 || idx >= MAX_SHADER_PROGRAMS) return "<unknown>";
    return kShaderNames[idx];
}

const char *MikuPan_GetShaderVertSource(int idx)
{
    if (idx < 0 || idx >= MAX_SHADER_PROGRAMS) return NULL;
...
static void CacheUniformLocations(int idx, GLuint program)
{
    ...
    uniform_loc[idx].uColorScale      = glad_glGetUniformLocation(program, "uColorScale");
...
/// Compile one stage from a file. On error, returns 0 and writes
/// `infolog` (space-truncated) into err_buf. The shader object is deleted.
                                   ...
                                   char *err_buf, int err_buf_size)
{
    ...
    if (src == NULL)
    {
        if (err_buf && err_buf_size > 0) snprintf(err_buf, err_buf_size, "OOM reading %s", path);
        return 0;
    }
    src[file_size - 1] = 0;
    ...
    if (!ok)
    {
        if (err_buf && err_buf_size > 0)
        {
            ...
            {
                glad_glGetShaderInfoLog(sh, err_buf_size - written, NULL, err_buf + written);
            }
        }
...
static GLuint BuildShaderProgram(int idx, char *err_buf, int err_buf_size)
{
    ...
    GLuint vs = CompileStageFromFile(GL_VERTEX_SHADER, vert_path, err_buf, err_buf_size);
    if (!vs) return 0;

    GLuint fs = CompileStageFromFile(GL_FRAGMENT_SHADER, frag_path, err_buf, err_buf_size);
    if (!fs) { glad_glDeleteShader(vs); return 0; }

    GLuint gs = 0;
    if (geom_path != NULL)
    {
        gs = CompileStageFromFile(GL_GEOMETRY_SHADER, geom_path, err_buf, err_buf_size);
        if (!gs) { glad_glDeleteShader(vs); glad_glDeleteShader(fs); return 0; }
    }

    GLuint prog = glad_glCreateProgram();
    glad_glAttachShader(prog, vs);
    glad_glAttachShader(prog, fs);
    if (gs) glad_glAttachShader(prog, gs);
    glad_glLinkProgram(prog);

    glad_glDeleteShader(vs);
    glad_glDeleteShader(fs);
    if (gs) glad_glDeleteShader(gs);
    ...
    if (!ok)
    {
        if (err_buf && err_buf_size > 0)
        {
            ...
            if (written < err_buf_size - 1)
            {
                glad_glGetProgramInfoLog(prog, err_buf_size - written, NULL, err_buf + written);
            }
        }
...
int MikuPan_InitShaders()
{
    for (int i = 0 ; i < MAX_SHADER_PROGRAMS ; i++)
    {
        ...
        if (prog == 0)
        {
            // Initial build failure — keep going with whatever we already
            // had (zero for a fresh init, which will silently no-op rather
            // than crash). The reload path provides surfaced error messages.
            continue;
        }
        shader_list[i] = prog;
...
int MikuPan_ReloadShader(int idx, char *err_buf, int err_buf_size)
{
    if (idx < 0 || idx >= MAX_SHADER_PROGRAMS)
    {
        if (err_buf && err_buf_size > 0) snprintf(err_buf, err_buf_size, "bad shader index %d", idx);
        return -1;
    }
    ...
    if (old != 0) glad_glDeleteProgram(old);
