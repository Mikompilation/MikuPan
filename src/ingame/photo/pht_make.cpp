...
#define PHT_MAKE_BUFFER_SIZE 0x18010
...
void CompressData(int addri, int addro, int n)
{
    ...
    value = SlideEncode((u_char*) MikuPan_GetHostAddress(addri),
        (u_char*) MikuPan_GetHostAddress(addri + PHT_MAKE_BUFFER_SIZE),
        PHT_MAKE_BUFFER_SIZE);
    ...
    if (value > 0.549f)
    {
        ...
        while (1)
        {
            memset((void*) MikuPan_GetHostAddress(addri + PHT_MAKE_BUFFER_SIZE),
                   0xff, PHT_MAKE_BUFFER_SIZE);

            value = CompressFile(
                (u_short*) MikuPan_GetHostAddress(addri),
                (char*) MikuPan_GetHostAddress(addri + PHT_MAKE_BUFFER_SIZE),
                PHT_MAKE_BUFFER_SIZE, quality);
            ...
            if (quality >= 5)
            {
                addr = (u_int*) MikuPan_GetHostAddress(addri + PHT_MAKE_BUFFER_SIZE);
                addr[1] = 2;
    ...
    memcpy((void*) MikuPan_GetHostAddress(addro + one_size * n),
           (void*) MikuPan_GetHostAddress(addri + PHT_MAKE_BUFFER_SIZE),
           one_size);
}
...
void UncompressData(int addri, int n, int addro)
{
    ...
    switch (type)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            memset((void*) MikuPan_GetHostAddress(addro), 0, PHT_MAKE_BUFFER_SIZE);
