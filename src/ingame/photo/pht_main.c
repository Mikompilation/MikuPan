        ...
        case PHOTO_MODE_END:
            if (cnt == 1)
            {
                ...
                if (hint_2d != 0 && hint_2d_num < 100
                    && pazene_load_tbl[hint_2d_num] != -1)
                {
                    for (i = 0, fl = -1; i < 2 && fl == -1; i++)
                    {
                        if (((pazene_load_buf[i] & 0x80) != 0)
                            && ((pazene_load_buf[i] & 0x7f)
                                == pazene_load_tbl[hint_2d_num]))
                        {
                            fl = i;
                        }
                    }
                    ...
                    if (fl == -1)
                    {
                        for (i = 0, fl = -1; i < 2 && fl == -1; i++)
                        {
                            if (pazene_load_buf[i] == 0xff)
                            {
                                ...
                                pazene_load_buf[i] =
                                    pazene_load_tbl[hint_2d_num] | 0x80;
                                area_wrk.rgst[i] =
                                    pazene_load_tbl[hint_2d_num] | 0x80;
                        ...
                        if (fl != -1)
                        {
                            ...
                            load_id2 = LoadReq(pazz_textbl[i],
                                               0x1c90000 + fl * 0x10000);
                        }
                    }
                }
                ...
                if (photo_wrk.rare[0].no >= 0x80
                    && photo_wrk.rare[0].score != 0)
                {
                    PuzzleGhostPhotoReq();
                    InfoDispPause();
                ...
                for (i = 0; i < 5; i++)
                {
                    if (photo_wrk.furn[i].no != 0xffff
                        && photo_wrk.furn[i].type == 32)
                    {
                        if (PhotoEventOpenJudge(
                                furn_dat[photo_wrk.furn[i].no].fp_mak, 0xff)
                            != 0)
                        {
                            break;
                        }
                    }
                    else if (photo_wrk.furn[i].no != 0xffff
                             && photo_wrk.furn[i].type == 64)
                    {
                        if (PhotoEventOpenJudge(0xff, photo_wrk.furn[i].no)
                            != 0)
                        {
                            break;
                        }
