...
int ChkANDCondition(FURN_ACT_WRK *fawp, u_char **cpp, u_char count)
{
    ...
    while (--count != 0xff)
    {
        ...
        case 18:
            ...
            if (j == 0)
            {
                i = (MikuPan_Rand() / (float)MikuPan_RAND_MAX) * 100.0f;
...
void FSpeTexAnmExe(u_char modelnum, FURN_ACT_WRK *fawp, u_char regmode, u_char looptype, u_char speed)
{
    ...
    matp = (SgMaterial *)MikuPan_GetHostPointer(hs->matp);
    phead = (u_int *)MikuPan_GetHostPointer(hs->phead);
...
float GetRandVal(float min, float max)
{
    ...
    return min + (MikuPan_Rand() / (float)MikuPan_RAND_MAX) * width;
}
